...
function PlanWidget({ plan, since }: { plan: string; since: string | null }) {
  ...
  return (
    <Link href="/dashboard/plan" style={{ display: "block", textDecoration: "none", padding: "10px 11px", background: m.bg, border: `1px solid ${m.border}`, borderRadius: 10, cursor: "pointer", transition: "all .15s" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: sinceStr ? 6 : 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 10, fontFamily: "'DM Sans',sans-serif", color: m.color, letterSpacing: ".10em", fontWeight: 700 }}>{m.label}</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <span style={{ fontSize: 9, fontFamily: "'DM Sans',sans-serif", color: "rgba(0,229,160,.7)", letterSpacing: ".06em" }}>AL CORRIENTE</span>
        </div>
      </div>
      {sinceStr && (
        <div style={{ fontSize: 9, color: "rgba(238,242,255,.28)", fontFamily: "'DM Sans',sans-serif", letterSpacing: ".04em" }}>
          Desde {sinceStr}
        </div>
      )}
    </Link>
...
function DashboardInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600;9..40,700&family=Fraunces:opsz,wght@9..144,400;9..144,500;9..144,600&family=Outfit:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500;600&display=swap');
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nl.on    { background: #173463; color: #fff; font-weight: 600; }
        .nl.on .ico { color: #00E5A0; }
        ...
        .sec-title {
          ...
          text-transform: uppercase; letter-spacing: 0.1em; color: #5E729B;
          padding: 0 14px; margin-top: 20px; margin-bottom: 6px;
        }
        .sec-title:first-child { margin-top: 4px; }

        /* ── Content area typography ── */
        .dash-content { font-family: 'Outfit', sans-serif; font-size: 14px; color: #0F172A; }
        .dash-content h1 { font-family: 'Fraunces', serif; font-weight: 600; font-size: 34px; letter-spacing: -0.01em; color: #0F172A; line-height: 1.2; margin: 0; }
        .dash-content h2 { font-family: 'Outfit', sans-serif; font-weight: 600; font-size: 18px; color: #0F172A; margin: 0; }
        .dash-content p, .dash-content span, .dash-content td, .dash-content th, .dash-content li, .dash-content label, .dash-content div { font-family: inherit; }
        .dash-content code, .dash-content [data-mono] { font-family: 'IBM Plex Mono', monospace; }
      `}</style>
      ...
      <aside className="sb" style={{ width: W }}>
        ...
        <nav style={{ flex: 1, padding: "10px 8px", display: "flex", flexDirection: "column", gap: 0, overflowY: "auto", position: "relative", zIndex: 1 }}>
          {NAV_GROUPS.map((group, gi) => (
            <React.Fragment key={group.title}>
              {open && <div className="sec-title" style={gi === 0 ? { marginTop: 4 } : undefined}>{group.title}</div>}
              {!open && gi > 0 && <div style={{ height: 1, background: "rgba(255,255,255,.07)", margin: "8px 10px" }} />}
              {group.items.map(n => {
      ...
      <div className="dash-content" style={{ marginLeft: W, minHeight: "100vh", background: "#F4F6FB", transition: "margin-left .25s cubic-bezier(.16,1,.3,1)" }}>
        <div style={{ maxWidth: pathname?.startsWith("/dashboard/calculadora") ? "none" : 1200, padding: pathname?.startsWith("/dashboard/calculadora") ? "0" : "28px 32px 56px" }}>
          {children}
        </div>
      </div>
    </>
