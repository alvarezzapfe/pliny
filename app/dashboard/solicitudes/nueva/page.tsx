...
const CLIENTES_MOCK = [
  { id: "cli_001", empresa: "Grupo Textil Monterrey SA de CV", rfc: "GTM210405AB2" },
  { id: "cli_002", empresa: "Distribuidora del Pacífico SAPI", rfc: "DPA190830CD5" },
  { id: "cli_003", empresa: "Constructora Altamira SC",        rfc: "CAL200115EF8" },
  { id: "cli_004", empresa: "TechLogix México SA de CV",       rfc: "TLM220601GH1" },
];
...
export default function NuevaSolicitudPage() {
  ...
  const filteredClients = CLIENTES_MOCK.filter(c =>
    c.empresa.toLowerCase().includes(search.toLowerCase()) ||
    c.rfc.toLowerCase().includes(search.toLowerCase())
  );

  const selectedClient = CLIENTES_MOCK.find(c => c.id === form.clienteId);
  ...
  const empresaLabel = form.clienteMode === "existente"
    ? selectedClient?.empresa || "—"
    : form.empresa || "—";

  const rfcLabel = form.clienteMode === "existente"
    ? selectedClient?.rfc || "—"
    : form.rfc || "—";

  if (submitted) {
    return (
      <SuccessScreen empresa={empresaLabel} tipo={form.tipo} monto={form.monto} />
    );
  }
    ...
    <div style={{ fontFamily: "'Geist',sans-serif", color: "#0F172A", maxWidth: 780, margin: "0 auto" }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .inp, .sel, .textarea {
          ...
          padding:0 14px; font-family:'Geist',sans-serif; font-size:13px; color:#0F172A;
        ...
        .cli-row {
          ...
          border:1.5px solid #E8EDF5; border-radius:11px; cursor:pointer;
        ...
        .btn-ghost {
          ...
          background:transparent; color:#475569; border:1px solid #E8EDF5;
        ...
        .success-ring { animation:successRing .5s cubic-bezier(.16,1,.3,1) both; }
        .check-pop { animation:checkPop .4s cubic-bezier(.16,1,.3,1) .3s both; }
      `}</style>
          ...
          <React.Fragment key={label}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
              <div className={`step-dot ${i < step ? "done" : i === step ? "active" : "idle"}`}>
                {i < step
                  ? <Ic d="M3 8l3.5 3.5L13 4" s={12} c="#fff" sw={2} />
                  : i + 1
                }
              </div>
              <span style={{
                fontSize: 10, fontWeight: i === step ? 700 : 500,
                color: i <= step ? "#0C1E4A" : "#94A3B8",
      ...
      <div style={{ height: 3, background: "#F1F5F9", borderRadius: 999, marginTop: 20, overflow: "hidden" }}>
        <div style={{
          ...
          background: "linear-gradient(90deg,#1B3F8A,#5B8DEF)",
          width: `${((step + 1) / 3) * 100}%`,
  ...
  return (
    <div className="card scale-in" style={{ padding: 24 }}>
      ...
      <div style={{ display: "flex", background: "#F8FAFC", borderRadius: 10, padding: 3, marginBottom: 22, border: "1px solid #E8EDF5" }}>
        <button className={`tab-btn ${form.clienteMode === "existente" ? "active" : ""}`}
          onClick={() => set("clienteMode", "existente")}>
          Cliente existente
        </button>
        <button className={`tab-btn ${form.clienteMode === "nuevo" ? "active" : ""}`}
          onClick={() => set("clienteMode", "nuevo")}>
          + Nuevo cliente
        </button>
      </div>
      ...
      {form.clienteMode === "existente" ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {filteredClients.length === 0 && (
              <div style={{ textAlign: "center", padding: "28px 0", color: "#94A3B8", fontSize: 13 }}>
                Sin resultados. Prueba con otro nombre o RFC.
              </div>
            )}
            {filteredClients.map((c: any) => (
              <div key={c.id}
                className={`cli-row ${form.clienteId === c.id ? "selected" : ""}`}
                onClick={() => set("clienteId", c.id)}>
                <div style={{
                  width: 36, height: 36, borderRadius: 9,
                  background: form.clienteId === c.id ? "linear-gradient(135deg,#0C1E4A,#1B3F8A)" : "#EEF2FF",
                  display: "grid", placeItems: "center", flexShrink: 0,
                }}>
                  <Ic d="M8 2a4 4 0 100 8A4 4 0 008 2zM2 14c0-2.2 2.7-4 6-4s6 1.8 6 4" s={16}
                    c={form.clienteId === c.id ? "#fff" : "#5B8DEF"} />
                </div>
                <div style={{ flex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={{ gridColumn: "1/-1" }}>
            ...
            <input className="inp" placeholder="Empresa SA de CV" value={form.empresa}
              onChange={e => set("empresa", e.target.value)} />
          </div>
          <div>
            <label className="lbl">RFC</label>
            <input className="inp mono" placeholder="XAXX010101000" value={form.rfc}
              onChange={e => set("rfc", e.target.value.toUpperCase())} maxLength={13} />
          </div>
          <div>
        ...
        {TIPOS.map(t => (
          <div key={t.value}
            ...
            <div style={{
              ...
              background: form.tipo === t.value ? "linear-gradient(135deg,#0C1E4A,#1B3F8A)" : "#EEF2FF",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div>
          ...
          <div style={{ position: "relative" }}>
            ...
            <input className="inp mono" style={{ paddingLeft: 26 }}
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <input className="inp mono" placeholder="24" value={form.plazo}
              onChange={e => set("plazo", e.target.value.replace(/\D/g, ""))}
              style={{ flex: 1 }} maxLength={3} />
            <select className="sel" value={form.plazoUnidad}
              onChange={e => set("plazoUnidad", e.target.value)}
        ...
        <div>
          ...
          <div style={{ position: "relative" }}>
            <input className="inp mono" placeholder="TIIE + 3.5" value={form.tasaReferencia}
              onChange={e => set("tasaReferencia", e.target.value)} />
          </div>
        </div>
        <div>
          <label className="lbl">Garantía</label>
          <input className="inp" placeholder="Hipotecaria, prendaria..." value={form.garantia}
            onChange={e => set("garantia", e.target.value)} />
        </div>
        <div style={{ gridColumn: "1/-1" }}>
          <label className="lbl">Destino del crédito <span style={{ color: "#F43F5E" }}>*</span></label>
          <textarea className="textarea" placeholder="Describe brevemente para qué se utilizarán los recursos..."
...
function SuccessScreen({ empresa, tipo, monto }: { empresa: string; tipo: string; monto: string }) {
  return (
    <div style={{
      fontFamily: "'Geist',sans-serif", color: "#0F172A",
      ...
      <div style={{ textAlign: "center", maxWidth: 400 }}>
        <div className="ring" style={{
          ...
          boxShadow: "0 0 0 12px rgba(91,141,239,.12)",
        }}>
          <div className="check">
            <Ic d="M3 8l3.5 3.5L13 4" s={28} c="#fff" sw={2} />
          </div>
        </div>
        <div className="fade d1" style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-.04em", marginBottom: 8 }}>
          ¡Solicitud enviada!
        </div>
        ...
        <div className="fade d2" style={{
          ...
          padding: "4px 12px", fontSize: 11, color: "#92400E", marginBottom: 24,
        ...
        <div className="fade d3" style={{ display: "flex", gap: 10, justifyContent: "center" }}>
          <Link href="/dashboard/solicitudes/nueva" className="btn-ghost">
            Nueva solicitud
          </Link>
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
      <div style={{
        ...
        background: "linear-gradient(135deg,rgba(12,30,74,.07),rgba(27,63,138,.12))",
