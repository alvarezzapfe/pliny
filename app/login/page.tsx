...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error: oauthErr } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `https://www.plinius.mx/auth/callback`,
      },
    });
    if (oauthErr) {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (cleanEmail.length < 5 || !cleanEmail.includes("@")) {
      setLoading(false); setError("Correo inválido."); return;
    }
    if (password.length < 6) {
      setLoading(false); setError("Contraseña muy corta."); return;
    }
    ...
    >
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          --blue-deep:#0C1E4A; --blue-mid:#0F2254; --blue-rich:#1B3F8A;
          --accent:#5B8DEF; --green:#00E5A0;
      ...
      <section className="left-panel" style={{ position:"relative", overflow:"hidden", background:"radial-gradient(ellipse 120% 80% at 25% 10%,#1B3F8A 0%,#0C1E4A 55%,#091530 100%)", padding:"40px 48px", display:"flex", flexDirection:"column", justifyContent:"space-between" }}>
        ...
        <div style={{ position:"relative", zIndex:2 }}>
          ...
          <h1 style={{ fontSize:42, fontWeight:800, lineHeight:1.06, letterSpacing:"-0.04em", color:"var(--fg)", marginBottom:14 }}>
            Underwrite.<br /><span style={{ color:"var(--green)" }}>Monitor.</span>{" "}<span style={{ color:"var(--fg-3)", fontWeight:600 }}>Report.</span>
          </h1>
        ...
        <div style={{ position:"relative", zIndex:2 }}>
          ...
          <div style={{ fontFamily:"'Geist Mono',monospace", fontSize:10, color:"var(--fg-3)" }}>© {new Date().getFullYear()} Plinius · {hint}</div>
        </div>
      </section>
      ...
      <section className="right-panel" style={{ background:"#F8FAFC", position:"relative", overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 32px" }}>
        ...
        <div style={{ width:"100%", maxWidth:420, position:"relative", zIndex:1 }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
            ...
            <Link href="/admin/login" style={{ fontSize:12, fontWeight:600, color:"#64748B", textDecoration:"none" }}>Admin →</Link>
          </div>
          ...
          <div className="card">
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22 }}>
              <div>
                <h2 style={{ fontSize:21, fontWeight:700, color:"#0F172A", letterSpacing:"-0.03em", marginBottom:4 }}>Bienvenido de nuevo</h2>
                <p style={{ fontSize:13, color:"#64748B" }}>Accede a tu consola de crédito.</p>
              </div>
            ...
            <form onSubmit={onSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <div>
                ...
                <div className="inp-wrap">
                  <div className={`inp-icon${emailFocus ? " focused" : ""}`}>
                    <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><path d="M1.5 3.5l6 4.5 6-4.5M1.5 3.5h12v8h-12z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round"/></svg>
                  </div>
                  <input value={email} onChange={e=>setEmail(e.target.value)} onFocus={()=>setEmailFocus(true)} onBlur={()=>setEmailFocus(false)} type="email" required autoComplete="email" placeholder="correo@empresa.com" className={`inp${error ? " has-error" : ""}`} />
                </div>
              </div>
              ...
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:7 }}>
                  ...
                  <Link href="/forgot-password" style={{ fontSize:11, fontWeight:600, color:"#5B8DEF", textDecoration:"none" }}>¿Olvidaste la contraseña?</Link>
                </div>
                <div className="inp-wrap">
                  <div className={`inp-icon${passwordFocus ? " focused" : ""}`}>
                    <svg width="15" height="15" viewBox="0 0 15 15" fill="none"><rect x="2.5" y="6.5" width="10" height="7" rx="1.5" stroke="currentColor" strokeWidth="1.2"/><path d="M4.5 6.5V5a3 3 0 016 0v1.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>
                  </div>
                  <input value={password} onChange={e=>setPassword(e.target.value)} onFocus={()=>setPasswordFocus(true)} onBlur={()=>setPasswordFocus(false)} type={showPass ? "text" : "password"} required autoComplete="current-password" placeholder="••••••••" className={`inp${error ? " has-error" : ""}`} />
              ...
              <button type="submit" disabled={isAnyLoading} className="btn-submit" style={{ marginTop:2 }}>
                ...
                {loading ? (
                  ...
                  <>Entrar a la consola<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2.5 6.5h8M7 3.5l3 3-3 3" stroke="rgba(255,255,255,0.65)" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg></>
                )}
              </button>

              <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:12, color:"#94A3B8" }}>¿Nuevo aquí?</span>
                <Link href="/register" style={{ fontSize:12, fontWeight:600, color:"#0F172A", textDecoration:"none", borderBottom:"1px solid #E2E8F0", paddingBottom:1 }}>Crear cuenta →</Link>
              </div>
            </form>
          </div>
