...
interface SolicitudForm {
  ...
  descripcion: string;
  plazo_meses: string;
  tasa_solicitada: string;
  amortizacion: string;
  garantia_tipo: string;
  garantia_detalle: string;
    ...
    <div onClick={onClick} style={{
      ...
      {icon && (
        <div style={{ width:36, height:36, borderRadius:9, background: selected ? "rgba(5,150,105,.15)" : "#F8FAFC", display:"grid", placeItems:"center", flexShrink:0 }}>
          <Ic d={icon} s={16} c={selected ? "#059669" : "#94A3B8"}/>
        </div>
      )}
      <div style={{ flex:1 }}>
          ...
          <React.Fragment key={s.n}>
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
              <div style={{
                ...
                background: state==="done" ? "#059669" : state==="active" ? "#064E3B" : "#F1F5F9",
                color: state==="idle" ? "#94A3B8" : "#fff",
                boxShadow: state==="active" ? "0 0 0 4px rgba(5,150,105,.2)" : "none",
                transition:"all .2s",
              }}>
                {state==="done"
            ...
            {i < STEPS.length-1 && (
              <div style={{ flex:1, height:2, background: s.n < step ? "#059669" : "#E2E8F0", margin:"0 4px", marginBottom:16, transition:"background .3s" }}/>
            )}
          </React.Fragment>
        );
      })}
...
export default function SolicitudesPage() {
  ...
  function upd(k: keyof SolicitudForm, v: string) {
    setForm(f => ({ ...f, [k]: v }));
  }
            ...
            {solicitudes.map((s: any) => {
              ...
              return (
                <div key={s.id} className="tr" style={{ gridTemplateColumns:"1fr 110px 80px 100px 110px" }}>
                  ...
                  <span style={{ display:"inline-flex", alignItems:"center", borderRadius:999, padding:"3px 9px", fontSize:10, fontWeight:600, fontFamily:"'Geist Mono',monospace", background:pill.bg, color:pill.color, border:`1px solid ${pill.border}` }}>
                    {pill.label}
                  </span>
