...
// sin exponer el API key del otorgante en el cliente
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const { data: lender } = await supabase
    .from('onb_lenders')
    .select('id, slug, name, primary_color, secondary_color, logo_url, api_key_hash, active')
    .eq('slug', slug)
    .eq('active', true)
    .single()
  ...
  return NextResponse.json({
    ...
    headers: {
      'Cache-Control': 'no-store', // nunca cachear tokens
    }
  })
}
